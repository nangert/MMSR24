...
class MFCCRetrievalSystem:
    ...
    def __init__(self, dataset, similarity_cache_path_merged: str = 'precomputed/mfcc_similarities.pkl', similarity_cache_path_bow: str = 'precomputed/mfcc_similarities_bow.pkl', similarity_cache_path_stat: str = 'precomputed/mfcc_similarities_stat.pkl', similarity_cache_path_bow_cos: str = 'precomputed/mfcc_similarities_bow_cos.pkl', similarity_cache_path_stat_cos: str = 'precomputed/mfcc_similarities_stat_cos.pkl'):
        """
        ...
        self.similarities_bow_cos = self.load_or_compute_similarities_bow_cos()
        self.similarities_stat = self.load_or_compute_similarities_stat()
        self.similarities_stat_cos = self.load_or_compute_similarities_stat_cos()
    ...
    def load_or_compute_similarities_bow_cos(self) -> dict:
        """
        Loads precomputed similarities from a file if it exists, otherwise computes and saves them.
    ...
    def load_or_compute_similarities_stat_cos(self) -> dict:
        """
        Loads precomputed similarities from a file if it exists, otherwise computes and saves them using Mahalanobis distance.
    ...
    def recommend_similar_songs_bow_cos(self, query_song, k: int = 5) -> list:
        ...
        """
        query_id = query_song.song_id
        if query_id not in self.similarities_bow_cos:
            raise ValueError(f"No precomputed similarities found for song ID: {query_id}")

        similarities_with_songs = []
        for song_id, similarity in self.similarities_bow_cos[query_id].items():
            song = self.song_dict.get(song_id)
            if song:
                similarities_with_songs.append((song, similarity))
    ...
    def recommend_similar_songs_stat_cos(self, query_song, k: int = 5) -> list:
        ...
        """
        query_id = query_song.song_id
        if query_id not in self.similarities_stat_cos:
            raise ValueError(f"No precomputed similarities found for song ID: {query_id}")

        similarities_with_songs = []
        for song_id, similarity in self.similarities_stat_cos[query_id].items():
            song = self.song_dict.get(song_id)
            if song:
                similarities_with_songs.append((song, similarity))
